...
class FastAttention(nn.Module):
    def __init__(self, dim_heads, nb_features=None, ortho_scaling=0, causal=False, generalized_attention=False,
                 kernel_fn=nn.ReLU(), no_projection=False, feature_redraw_interval=1000):
        super().__init__()
...
class PerformerAttention(nn.Module):
    def __init__(
        ...
        self.fast_attention = FastAttention(dim_head, nb_features, causal=causal,
                                            ...
                                            no_projection=no_projection, feature_redraw_interval=feature_redraw_interval)
...
class PerformerBlock(Block):
    def __init__(self, dim, num_heads, feature_redraw_interval=1000, attn_drop=0., qkv_bias=False,
                 generalized_attention=True, kernel_fn=nn.ReLU(), nb_features=None, **kwargs):
        super().__init__(dim=dim, num_heads=num_heads, qkv_bias=qkv_bias, **kwargs)
        self.attn = PerformerAttention(dim, heads=num_heads, feature_redraw_interval=feature_redraw_interval,
                                       dropout=attn_drop, qkv_bias=qkv_bias, nb_features=nb_features,
                                       generalized_attention=generalized_attention, kernel_fn=kernel_fn)


class PerformerViT(TimmViT):
    def __init__(self, feature_redraw_interval=1000, generalized_attention=False, kernel_fn=nn.ReLU(), nb_features=None, **kwargs):
        super().__init__(block_fn=partial(PerformerBlock, feature_redraw_interval=feature_redraw_interval,
                                          nb_features=nb_features, generalized_attention=generalized_attention, kernel_fn=kernel_fn), **kwargs)
...
def performer_vit_tiny_patch16(pretrained=False, img_size=224, **kwargs):
    ...
    sizes = vit_sizes["Ti"]
    model = PerformerViT(img_size=img_size, patch_size=16, in_chans=3, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                         feature_redraw_interval=None, nb_features=128, **sizes, **kwargs)
    return model
