...
class SwinTransformer(nn.Module, ResizingInterface):
    ...
    def set_image_res(self, res):
        ...
        for i_layer, layer in enumerate(self.layers):
            ...
            for block in layer.blocks:
                ...
                if block.shift_size > 0:
                    ...
                    img_mask = torch.zeros((1, H, W, 1))  # 1 H W 1
                    h_slices = (slice(0, -block.window_size),
                                slice(-block.window_size, -block.shift_size),
                                slice(-block.shift_size, None))
                    w_slices = (slice(0, -block.window_size),
                                slice(-block.window_size, -block.shift_size),
                                slice(-block.shift_size, None))
                    cnt = 0
                    for h in h_slices:
        ...
        if self.ape:
            orig_size = int((self.absolute_pos_embed.shape[-2]) ** .5)
            ...
            pos_tokens = nn.functional.interpolate(pos_tokens, size=(new_size, new_size), mode='bicubic',
                                                   align_corners=False)
...
swin_sizes = {"Ti": dict(embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24]),
