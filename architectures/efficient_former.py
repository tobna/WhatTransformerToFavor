            ...
            if isinstance(stage, nn.Sequential):
                for part in stage:
                    if isinstance(part, AttnFFN):
                        block_res = (
                            eformer_block_res
                            if part.token_mixer.stride is None
                            else math.ceil(eformer_block_res / part.token_mixer.stride)
                        )
                        old_block_res = part.token_mixer.resolution
                        print(
                            f"AttnFFN res: {old_block_res} -> {block_res}, N: {part.token_mixer.N} -> {block_res ** 2}, N2: {part.token_mixer.N2} -> {block_res ** 2}"
                        )
                ...
                if stage.asub and not stage.light:
                    ...
                    points_ = list(itertools.product(range(stage.attn.resolution2), range(stage.attn.resolution2)))
                    N = len(points)
                    N_ = len(points_)
                    attention_offsets = {}
                    idxs = []
                    for p1 in points_:
                        for p2 in points:
                            ...
                            offset = (
                                abs(
                                    p1[0] * math.ceil(stage.attn.resolution / stage.attn.resolution2)
                                    - p2[0]
                                    + (size - 1) / 2
                                ),
                                abs(
                                    p1[1] * math.ceil(stage.attn.resolution / stage.attn.resolution2)
                                    - p2[1]
                                    + (size - 1) / 2
                                ),
                            )
                    ...
                    biases = F.interpolate(
                        biases, size=(eformer_block_res, eformer_block_res), mode="bicubic", align_corners=False
                    )
...
@register_model
def efficientformerv2_s0(pretrained=False, img_size=224, drop_path_rate=0.0, **kwargs):
...
@register_model
def efficientformerv2_s1(pretrained=False, img_size=224, drop_path_rate=0.0, **kwargs):
    model = EfficientFormerV2(
        img_size=img_size,
...
@register_model
def efficientformerv2_s2(pretrained=False, img_size=224, drop_path_rate=0.02, **kwargs):
    model = EfficientFormerV2(
        img_size=img_size,
...
@register_model
def efficientformerv2_l(pretrained=False, img_size=224, drop_path_rate=0.1, **kwargs):
    model = EfficientFormerV2(
        img_size=img_size,
