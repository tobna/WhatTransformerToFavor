...
class Attention4D(torch.nn.Module):
    ...
    def forward(self, x):  # x (B,N,C)
        ...
        # print(f"x shape: {x.shape}; B={B}, C={C}, H={H}, W={W}, num_heads={self.num_heads}, d={self.d}, dh={self.dh}")
...
class EfficientFormerV2(nn.Module, ResizingInterface):
    def __init__(self, layers, embed_dims=None,
        ...
        for i in range(len(layers)):
            stage = eformer_block(embed_dims[i], i, layers,
                                  ...
                                  use_layer_scale=use_layer_scale,
                                  layer_scale_init_value=layer_scale_init_value,
                                  resolution=math.ceil(img_size / (2 ** (i + 2))),
                                  vit_num=vit_num,
                                  e_ratios=e_ratios)
    ...
    def set_image_res(self, res):
        ...
        i_diff = 0
        for i, stage in enumerate(self.network):
            # adding an Embedding layer happens on the same i as the Sequential block before that...
            if isinstance(stage, Embedding):
                i_diff += 1
            i = i-i_diff
            eformer_block_res = math.ceil(res / (2 ** (i + 2)))
            if isinstance(stage, nn.Sequential):
                for part in stage:
                    if isinstance(part, AttnFFN):
                        block_res = eformer_block_res if part.token_mixer.stride is None else \
                            math.ceil(eformer_block_res / part.token_mixer.stride)
                        old_block_res = part.token_mixer.resolution
                        print(f"AttnFFN res: {old_block_res} -> {block_res}, N: {part.token_mixer.N} -> {block_res ** 2}, N2: {part.token_mixer.N2} -> {block_res ** 2}")
                        part.token_mixer.resolution = block_res
                        part.token_mixer.N = block_res ** 2
                        part.token_mixer.N2 = block_res ** 2

                        points = list(itertools.product(range(block_res), range(block_res)))
                        N = part.token_mixer.N
                        attention_offsets = {}
                        idxs = []
                        for p1 in points:
                            for p2 in points:
                                ...
                                if offset not in attention_offsets:
                                    attention_offsets[offset] = len(attention_offsets)
                                idxs.append(attention_offsets[offset])
                        # len attention offsets is block_res ** 2
                        biases = part.token_mixer.attention_biases.reshape(1, -1, old_block_res, old_block_res)
                        biases = F.interpolate(biases, size=(block_res, block_res), mode='bicubic', align_corners=False)
                        part.token_mixer.attention_biases = nn.Parameter(biases.reshape(-1, N))
                        part.token_mixer.register_buffer('attention_bias_idxs', torch.LongTensor(idxs).view(N, N))
            elif isinstance(stage, Embedding):
                if stage.asub and not stage.light:
                    old_block_res = stage.attn.resolution
                    stage.attn.resolution = eformer_block_res
                    stage.attn.resolution2 = math.ceil(eformer_block_res / 2)
                    stage.attn.q.resolution1 = eformer_block_res
                    stage.attn.q.resolution2 = stage.attn.resolution2
                    stage.attn.N = eformer_block_res ** 2
                    stage.attn.N2 = stage.attn.resolution2 ** 2

                    points = list(itertools.product(range(stage.attn.resolution), range(stage.attn.resolution)))
                    points_ = list(itertools.product(
                        range(stage.attn.resolution2), range(stage.attn.resolution2)))
                    N = len(points)
                    N_ = len(points_)
                    attention_offsets = {}
                    idxs = []
                    for p1 in points_:
                        for p2 in points:
                            size = 1
                            offset = (
                                abs(p1[0] * math.ceil(stage.attn.resolution / stage.attn.resolution2) - p2[0] + (size - 1) / 2),
                                abs(p1[1] * math.ceil(stage.attn.resolution / stage.attn.resolution2) - p2[1] + (size - 1) / 2))
                            if offset not in attention_offsets:
                                attention_offsets[offset] = len(attention_offsets)
                            idxs.append(attention_offsets[offset])
                    # len attention offsets is also block_res ** 2!
                    biases = stage.attn.attention_biases.reshape(1, -1, old_block_res, old_block_res)
                    biases = F.interpolate(biases, size=(eformer_block_res, eformer_block_res), mode='bicubic', align_corners=False)
                    stage.attn.attention_biases = nn.Parameter(biases.reshape(-1, N))
                    stage.attn.register_buffer('attention_bias_idxs', torch.LongTensor(idxs).view(N_, N))
                    print(f"Embedding res: {old_block_res} -> {eformer_block_res}")
...
@register_model
def efficientformerv2_s0(pretrained=False, img_size=224, drop_path_rate=0., **kwargs):
...
@register_model
def efficientformerv2_s1(pretrained=False, img_size=224, drop_path_rate=0., **kwargs):
    model = EfficientFormerV2(
        img_size=img_size,
...
@register_model
def efficientformerv2_s2(pretrained=False, img_size=224, drop_path_rate=.02, **kwargs):
    model = EfficientFormerV2(
        img_size=img_size,
...
@register_model
def efficientformerv2_l(pretrained=False, img_size=224, drop_path_rate=.1, **kwargs):
    model = EfficientFormerV2(
        img_size=img_size,
