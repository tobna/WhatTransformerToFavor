...
class ConvolutionalVisionTransformer(nn.Module, ResizingInterface):
    def __init__(self, patch_size=[7, 3, 3], in_chans=3, num_heads=[1, 3, 6], num_classes=1000, act_layer=nn.GELU, norm_layer=nn.LayerNorm,
                 init='trunc_norm', patch_stride=[4, 2, 2], patch_padding=[2, 1, 1], embed_dim=[64, 192, 384],
                 depth= [1, 4, 16], mlp_ratio=4., qkv_bias=True, cls_token=[False, False, True], drop_rate=0.,
                 drop_path_rate=0., attn_drop_rate=0., method='dw_bn', kernel_size=3, padding_q=1, padding_kv=1,
                 stride_q=1, stride_kv=2, num_stages=3, **kwargs):
        super().__init__()
...
def cvt_w24(img_size=224, **kwargs):
    model = ConvolutionalVisionTransformer(patch_size=[7, 3, 3], depth=[2, 2, 20], embed_dim=[192, 768, 1024],
                                           num_heads=[3, 12, 16], pos_embed=False, **kwargs)
    return model
