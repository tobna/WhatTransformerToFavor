...
class TokenLearnerViT(TimmViT):
    def __init__(self, img_size=224, patch_size=16, token_learner_layer_rel=.5, learned_tokens=8, embed_dim=768,
                 mlp_ratio=4., drop_rate=0., norm_layer=None, depth=12, **kwargs):
        norm_layer = norm_layer if norm_layer is not None else partial(nn.LayerNorm, eps=1e-6)
        super().__init__(img_size=img_size, patch_size=patch_size, embed_dim=embed_dim, mlp_ratio=mlp_ratio, 
                         drop_rate=drop_rate, norm_layer=norm_layer, depth=depth, **kwargs)
        token_learner = TokenLearnerModuleV11(learned_tokens, embed_dim, bottleneck_dim=int(mlp_ratio * embed_dim),
                                              num_prefix_tokens=self.num_prefix_tokens, drop=drop_rate,
...
def token_learner_vit_8_50_tiny_patch16(pretrained=False, img_size=224, **kwargs):
    if 'layer_scale_init_values' in kwargs:
        kwargs['init_values'] = kwargs['layer_scale_init_values'] if 'layer_scale' in kwargs and kwargs['layer_scale'] \
            else None
    sizes = vit_sizes["Ti"]
    model = TokenLearnerViT(img_size=img_size, patch_size=16, in_chans=3, learned_tokens=8, token_learner_layer_rel=.5,
                            norm_layer=partial(nn.LayerNorm, eps=1e-6), **sizes, **kwargs)
    return model
...
def token_learner_vit_8_75_tiny_patch16(pretrained=False, img_size=224, **kwargs):
    ...
    sizes = vit_sizes["Ti"]
    model = TokenLearnerViT(img_size=img_size, patch_size=16, in_chans=3, learned_tokens=8, token_learner_layer_rel=.75,
                            norm_layer=partial(nn.LayerNorm, eps=1e-6), **sizes, **kwargs)
    return model
