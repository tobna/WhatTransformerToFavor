...
from timm.models.vision_transformer import LayerScale, Block, PatchEmbed
...
class LinformerSelfAttention(nn.Module):
    ...
    def resize(self, seq_len):
        ...
        proj_k_tokens, proj_k = proj_k[:, :, :extra_tokens], proj_k[:, :, extra_tokens:].view(1, self.k, old_imsize, old_imsize)
        proj_k = F.interpolate(proj_k, size=(new_imsize, new_imsize), mode='bilinear')  # 1 x k x imsize (new) x imsize (new)
        ...
        if not self.share_kv:
            ...
            proj_v_tokens, proj_v = proj_v[:, :, :extra_tokens], proj_v[:, :, extra_tokens:].view(1, self.k, old_imsize, old_imsize)
            proj_v = F.interpolate(proj_v, size=(new_imsize, new_imsize),
                                   mode='bilinear')  # 1 x k x imsize (new) x imsize (new)
...
class LinformerBlock(Block):
    def __init__(self, dim, num_heads, seq_len, k=256, mlp_ratio=4., drop=0., attn_drop=0., init_values=None,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, **kwargs):
        super().__init__(dim, num_heads, mlp_ratio=mlp_ratio, drop=drop, init_values=init_values, drop_path=drop_path,
                         act_layer=act_layer, norm_layer=norm_layer)
        self.attn = LinformerSelfAttention(dim, seq_len, k=k, heads=num_heads, dropout=attn_drop)


class LinformerViT(TimmViT):
    def __init__(self, img_size=224, patch_size=16, k=256, in_chans=3, num_classes=1000, global_pool='token',
                 embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., init_values=None, class_token=True,
                 no_embed_class=True, pre_norm=False, fc_norm=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
                 weight_init='', embed_layer=PatchEmbed, norm_layer=None, act_layer=None, **kwargs):
        ...
        super().__init__(img_size=img_size, patch_size=patch_size, in_chans=in_chans, num_classes=num_classes,
                         ...
                         mlp_ratio=mlp_ratio, init_values=init_values, class_token=class_token,
                         no_embed_class=no_embed_class, pre_norm=pre_norm, fc_norm=fc_norm, drop_rate=drop_rate,
                         attn_drop_rate=attn_drop_rate, drop_path_rate=drop_path_rate, weight_init=weight_init,
...
def linformer_vit_tiny_patch16(pretrained=False, img_size=224, **kwargs):
    ...
    sizes = vit_sizes["Ti"]
    model = LinformerViT(img_size=img_size, patch_size=16, in_chans=3, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                         **sizes, **kwargs)
    return model
