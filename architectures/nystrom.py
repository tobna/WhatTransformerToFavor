...
class NysAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,
                 num_landmarks=64, kernel_size=0, init_option="exact"):
        super().__init__()
    ...
    def forward(self, x, attn_mask=None):
        ...
        if (N % self.landmarks == 0):
            ...
            keys_landmarks_f = k[:, :, :num_k * segs, :].reshape(B, self.num_heads, num_k, segs, keys_head_dim).mean(
                dim=-2)
            keys_landmarks_l = k[:, :, num_k * segs:, :].reshape(B, self.num_heads, self.landmarks - num_k, segs + 1,
                                                                 keys_head_dim).mean(dim=-2)
            keys_landmarks = torch.cat((keys_landmarks_f, keys_landmarks_l), dim=-2)

            queries_landmarks_f = q[:, :, :num_k * segs, :].reshape(B, self.num_heads, num_k, segs, keys_head_dim).mean(
                dim=-2)
            queries_landmarks_l = q[:, :, num_k * segs:, :].reshape(B, self.num_heads, self.landmarks - num_k, segs + 1,
                                                                    keys_head_dim).mean(dim=-2)
        ...
        kernel_2 = torch.nn.functional.softmax(torch.matmul(queries_landmarks, keys_landmarks.transpose(-1, -2)),
                                               dim=-1)
...
class NystromBlock(Block):
    def __init__(self, dim, num_heads, qkv_bias=False, attn_drop=0., num_landmarks=64, kernel_size=0,
                 init_option="exact", proj_drop=0., **kwargs):
        super().__init__(dim, num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop, **kwargs)
        self.attn = NysAttention(dim, num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop,
                                 num_landmarks=num_landmarks, kernel_size=kernel_size, init_option=init_option)


class NystromViT(TimmViT):
    def __init__(self, *args, num_landmarks=64, init_option="exact", kernel_size=0, **kwargs):
        super().__init__(*args, block_fn=partial(NystromBlock, num_landmarks=num_landmarks, init_option=init_option,
...
def nystrom64_vit_tiny_patch16(pretrained=False, img_size=224, **kwargs):
    sizes = vit_sizes["Ti"]
    model = NystromViT(img_size=img_size, patch_size=16, in_chans=3, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                       num_landmarks=64, **sizes, **kwargs)
    return model
