...
# Licensed under The MIT.txt License [see LICENSE for details]
...
class FocalNet(nn.Module, ResizingInterface):
    ...
    def set_image_res(self, res):
        ...
        patches_resolution = [res // patch_size[0], res // patch_size[1]]
        self.patch_embed.img_size = res
        self.patch_embed.patches_resolution = patches_resolution
        self.patch_embed.num_patches = patches_resolution[0] * patches_resolution[1]
        self.img_size = res
...
def focalnet_tiny_srf(pretrained=False, **kwargs):
    model = FocalNet(depths=[2, 2, 6, 2], embed_dims=96, **kwargs)
    return model


@register_model
def focalnet_small_srf(pretrained=False, **kwargs):
    model = FocalNet(depths=[2, 2, 18, 2], embed_dims=96, **kwargs)
    return model


@register_model
def focalnet_base_srf(pretrained=False, **kwargs):
    model = FocalNet(depths=[2, 2, 18, 2], embed_dims=128, **kwargs)
    return model
...
def focalnet_tiny_iso(pretrained=False, **kwargs):
    model = FocalNet(depths=[12], patch_size=16, embed_dims=192, **kwargs)
    return model


@register_model
def focalnet_small_iso(pretrained=False, **kwargs):
    model = FocalNet(depths=[12], patch_size=16, embed_dims=384, **kwargs)
    return model
...
def focalnet_large_fl3(pretrained=False, **kwargs):
    model = FocalNet(depths=[2, 2, 18, 2], embed_dims=192, **kwargs)
    return model
...
def focalnet_xlarge_fl3(pretrained=False, **kwargs):
    model = FocalNet(depths=[2, 2, 18, 2], embed_dims=256, **kwargs)
    return model
...
def focalnet_huge_fl3(pretrained=False, **kwargs):
    model = FocalNet(depths=[2, 2, 18, 2], embed_dims=352, **kwargs)
    return model
