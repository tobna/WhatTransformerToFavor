# Taken from https://github.com/apple/ml-fastvit/blob/main/models/fastvit.py with slight changes
import torch
...
class AttentionBlock(Block):
    def __init__(self, dim, **kwargs):
        super().__init__(
            dim,
            mlp_layer=lambda in_features, hidden_features, act_layer=nn.GELU, drop=0.0: ConvFFN(
                in_channels=in_features, hidden_channels=hidden_features, act_layer=act_layer, drop=drop
            ),
            **kwargs,
        )
    ...
    def forward(self, x):
        ...
        x = self.attn(x)
        x = self.drop_path1(self.ls1(x.permute(0, 2, 1).reshape(B, C, H, W)))
    ...
    for block_idx in range(num_blocks[block_index]):
        ...
        if token_mixer_type == "repmixer":
            ...
            blocks.append(
                AttentionBlock(
                    dim,
                    num_heads=dim // 32,  # they force head_dim = 32
                    mlp_ratio=mlp_ratio,
                    act_layer=act_layer,
...
class FastViT(nn.Module, ResizingInterface):
    """
