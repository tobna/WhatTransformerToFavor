...
from routing_transformer.routing_transformer import (
    SelfAttention as LocalGlobalRouting,
    Kmeans,
    default,
    split_at_index,
    distribution,
...
class RoutingBlock(Block):
    def __init__(
        self, dim, max_seq_len, num_heads, num_local_heads, window_size, proj_drop=0.0, attn_drop=0.0, **kwargs
    ):
...
class RoutingViT(TimmViT):
    def __init__(self, img_size=224, patch_size=16, local_heads_fact=0.0, num_heads=12, class_token=True, **kwargs):
        assert img_size % patch_size == 0, (
            f"Patch size ({patch_size}) has to be divisible " f"by image resolution ({img_size})."
        )
        ...
        super().__init__(
            ...
            block_fn=partial(
                RoutingBlock, max_seq_len=window_size * rest_fact, num_local_heads=local_heads, window_size=window_size
            ),
            **kwargs,
        )
        self.window_size = window_size
    ...
    def set_image_res(self, res):
        ...
        print(
            f"Resizing: seq_len: {old_seq_len} |-> {new_seq_len}, "
        ...
        for block in self.blocks:
            ...
            if num_mem_kv > 0:
                old_mem_key = old_mem_key.permute(0, 3, 2, 1).reshape(
                    head_dim * num_heads, num_mem_kv, old_num_clusters
                )
                new_mem_key = (
                    nn.functional.interpolate(old_mem_key, size=new_num_clusters, mode="linear")
                    .reshape(num_heads, head_dim, num_mem_kv, new_num_clusters)
                    .permute(0, 3, 2, 1)
                )
                old_mem_value = old_mem_value.permute(0, 3, 2, 1).reshape(
                    head_dim * num_heads, num_mem_kv, old_num_clusters
                )
                new_mem_value = (
                    nn.functional.interpolate(old_mem_value, size=new_num_clusters, mode="linear")
                    .reshape(num_heads, head_dim, num_mem_kv, new_num_clusters)
                    .permute(0, 3, 2, 1)
                )
            ...
            new_means = nn.functional.interpolate(old_means, size=new_num_clusters, mode="linear").permute(0, 2, 1)
