...
def amp_diff_topk(x, k, temperature=1.0):
...
class SinkhornAttn(sinkhorn_transformer.SinkhornSelfAttention):
    def __init__(self, dim, num_heads, bucket_size, qkv_bias, attn_drop=0.0, proj_drop=0.0, **kwargs):
        super().__init__(
            dim,
            bucket_size,
            -1,
...
class SinkhornViT(CaiT):
    def __init__(self, img_size=224, patch_size=16, bucket_size_leq=32, sinkhorn_iter=8, temperature=0.75, **kwargs):
        assert img_size % patch_size == 0, "img_size has to be divisible by patch_size"
        bucket_size = _largest_div_leq((img_size // patch_size) ** 2, bucket_size_leq)
        print(f"Using bucket size {bucket_size}")
        super().__init__(
            attn_block=partial(
                SinkhornAttn, bucket_size=bucket_size, sinkhorn_iter=sinkhorn_iter, temperature=temperature
            ),
            img_size=img_size,
    ...
    def set_image_res(self, res):
        ...
        for block_layer in self.blocks:
            ...
            if block_layer.attn.n_local_attn_heads > 0:
                raise NotImplementedError("No bucket resizing for local attention implemented.")
