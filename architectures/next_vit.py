# Taken from https://github.com/bytedance/Next-ViT/blob/main/classification/nextvit.py with slight changes
import torch
...
class E_MHSA(nn.Module):
    ...
    def __init__(
        self, dim, out_dim=None, head_dim=32, qkv_bias=True, qk_scale=None, attn_drop=0, proj_drop=0.0, sr_ratio=1
    ):
        super().__init__()
        self.dim = dim
...
class NTB(nn.Module):
    ...
    def __init__(
        ...
        self.mhsa_out_channels = make_divisible(int(out_channels * mix_block_ratio), 32)
        self.mhca_out_channels = out_channels - self.mhsa_out_channels

        self.patch_embed = PatchEmbed(in_channels, self.mhsa_out_channels, stride)
        self.norm1 = norm_func(self.mhsa_out_channels)
        self.e_mhsa = E_MHSA(
            self.mhsa_out_channels, head_dim=head_dim, sr_ratio=sr_ratio, attn_drop=attn_drop, proj_drop=drop
        )
...
class NextViT(nn.Module, ResizingInterface):
    def __init__(
        self,
        stem_chs,
        depths,
        ...
        **kwargs
        ...
        self.head = nn.Linear(output_channel, num_classes) if num_classes > 0 else nn.Identity()
...
@register_model
def nextvit_small(pretrained=False, pretrained_cfg=None, dropout=0.0, drop_path_rate=0.0, **kwargs):
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], path_dropout=drop_path_rate, drop=dropout, **kwargs)
    return model


@register_model
def nextvit_base(pretrained=False, pretrained_cfg=None, dropout=0.0, drop_path_rate=0.0, **kwargs):
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 20, 3], path_dropout=drop_path_rate, drop=dropout, **kwargs)
    return model


@register_model
def nextvit_large(pretrained=False, pretrained_cfg=None, dropout=0.0, drop_path_rate=0.0, **kwargs):
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 30, 3], path_dropout=drop_path_rate, drop=dropout, **kwargs)
    return model
