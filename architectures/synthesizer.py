...
# The original code is licensed under the MIT license (see licenses/MIT.txt) from Tenzin Singhay Bhotia, 2020.
...
class FactorizedDenseAttention(nn.Module):
    def __init__(self, dim, num_heads, max_seq_len, f, qkv_bias=False, attn_drop=0.0, proj_drop=0.0):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        ...
        dense_attn = torch.repeat_interleave(a, self.max_seq_len // self.f, -1) * torch.repeat_interleave(
            b, self.f, -1
        )  # B x H x N x ms
...
class FactorizedDenseBlock(Block):
    def __init__(self, dim, num_heads, max_seq_len, f, qkv_bias=False, proj_drop=0.0, attn_drop=0.0, **kwargs):
        super().__init__(dim, num_heads, qkv_bias=qkv_bias, proj_drop=proj_drop, attn_drop=attn_drop, **kwargs)
        self.attn = FactorizedDenseAttention(
            dim, num_heads, max_seq_len, f, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop
        )
...
class FDSynthesizer(TimmViT):
    def __init__(self, img_size=224, patch_size=16, f=None, class_token=True, qkv_bias=False, **kwargs):
        ...
        super().__init__(
            ...
            qkv_bias=qkv_bias,
            block_fn=partial(FactorizedDenseBlock, max_seq_len=max_seq_len, f=f),
            **kwargs,
        )
        self.max_seq_len = max_seq_len
        self.f = f
    ...
    def set_image_res(self, res):
        ...
        for block in self.blocks:
            ...
            f_a_w = (
                F.interpolate(
                    block.attn.f_a.weight.view(1, num_heads * old_f, dim).transpose(1, 2),
                    size=num_heads * new_f,
                    mode="linear",
                    align_corners=False,
                )
                .view(dim, num_heads * new_f)
                .transpose(0, 1)
            )
            f_b_w = (
                F.interpolate(
                    block.attn.f_b.weight.view(1, num_heads * old_max_seq_len // old_f, dim).transpose(1, 2),
                    size=num_heads * new_max_seq_len // new_f,
                    mode="linear",
                    align_corners=False,
                )
                .view(dim, num_heads * new_max_seq_len // new_f)
                .transpose(0, 1)
            )
            ...
            if self.qkv_bias:
                ...
                f_a_b = F.interpolate(
                    block.attn.f_a.bias.view(1, 1, num_heads * old_f),
                    size=num_heads * new_f,
                    mode="linear",
                    align_corners=False,
                ).view(num_heads * new_f)
                f_b_b = F.interpolate(
                    block.attn.f_b.bias.view(1, 1, num_heads * old_max_seq_len // old_f),
                    size=num_heads * new_max_seq_len // new_f,
                    mode="linear",
                    align_corners=False,
...
class FactorizedRandomBlock(Block):
    def __init__(self, dim, num_heads, max_seq_len, f, qkv_bias=False, drop=0.0, attn_drop=0.0, **kwargs):
        super().__init__(dim, num_heads, qkv_bias=qkv_bias, drop=drop, attn_drop=attn_drop, **kwargs)
        self.attn = FactorizedRandomAttention(
            dim, num_heads, max_seq_len, f, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop
        )
...
class FRSynthesizer(TimmViT):
    ...
    def set_image_res(self, res):
        ...
        for block in self.blocks:
            r_1 = F.interpolate(
                block.attn.random_attn_1.transpose(1, 2), size=new_max_seq_len, mode="linear", align_corners=False
            ).transpose(1, 2)
...
def synthesizer_fd_vit_tiny_patch16(pretrained=False, img_size=224, f=None, **kwargs):
    ...
    sizes = vit_sizes["Ti"]
    model = FDSynthesizer(
        img_size=img_size, patch_size=16, in_chans=3, f=f, norm_layer=partial(nn.LayerNorm, eps=1e-6), **sizes, **kwargs
    )
    return model
