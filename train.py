...
import random
from math import cos, pi, isfinite, sqrt
...
def finetune(model, dataset, epochs, **kwargs):
    ...
    **kwargs : dict
    ...
    metrics = _train(model, train_loader, optimizer, rank, epochs, device, mixup, criterion, world_size, scheduler,
                     args, val_loader, val_criterion, model_folder, scaler, do_metrics_calculation=True,
                     show_tqdm=args.tqdm)

    ddp_cleanup()
    if rank == 0:
        logging.info(f"Run '{full_run_name}' is done.")
        save_model_state(model_folder, epochs, args, model_state=model.state_dict(),
                         stats=metrics, optimizer_state=optimizer.state_dict(),
                         scheduler_state=scheduler.state_dict(), additional_reason='final', regular_save=False)
...
def pretrain(model, dataset, epochs, **kwargs):
    ...
    **kwargs : dict
    ...
    if rank == 0:
        ...
        save_model_state(model_folder, epochs, args, model_state=model.state_dict(),
                         stats=metrics, optimizer_state=optimizer.state_dict(),
                         scheduler_state=scheduler.state_dict(), additional_reason='final')
...
def setup_tracking_and_logging(args, rank, append_model_path=None):
    ...
    if append_model_path is not None:
        ...
        elif isfile(args.logging_folder + '/' + append_model_path.split('/')[-3] + '_' + append_model_path.split('/')[-2] + '.log'):
    ...
    if full_run_name is None:
        full_run_name = f"{args.task.replace('-', '')}_{args.run_name}_{model}_{dataset}_" \
                        f"{datetime.now().strftime('%d.%m.%Y_%H:%M:%S')}"
    logging_file_name = f"{full_run_name}.log".replace("/", "_")
    logging.basicConfig(format=f"%(asctime)s [{args.run_name}/%(threadName)s -> {rank}] %(levelname)s: %(message)s",
                        datefmt="%d.%m.%Y %H:%M:%S", level=logging.INFO,
                        handlers=[logging.FileHandler(args.logging_folder + "/" + logging_file_name),
...
def setup_criteria_mixup(args):
    ...
    val_criterion = LabelSmoothingCrossEntropy(smoothing=0.)

    mixup = Mixup(mixup_alpha=0., cutmix_alpha=1., label_smoothing=args.label_smoothing, num_classes=args.n_classes)
    return criterion, val_criterion, mixup


def _train(model, train_loader, optimizer, rank, epochs, device, mixup, criterion, world_size, scheduler, args,
           val_loader, val_criterion, model_folder, scaler, do_metrics_calculation=True, start_epoch=0,
           show_tqdm=True, topk=(1, 5), acc_dict_key="acc{}"):
    ...
    for epoch in range(start_epoch, epochs):
        ...
        epoch_time, epoch_loss_rel, epoch_accs = _train_one_epoch(model, train_loader, optimizer, rank, epoch, device,
                                                                  mixup, criterion, world_size,
                                                                  scheduler, scaler, args, topk, acc_dict_key, show_tqdm)
        time_spend_training += epoch_time

        val_time, val_loss_rel, val_accs = _evaluate(model, val_loader, epoch, rank, device, world_size, val_criterion,
                                                     args, topk, acc_dict_key)
        ...
        if rank == 0 and (min(val_accs.values()) > current_best_acc or (epoch + 1) % args.save_epochs == 0):
            ...
            kwargs = dict(model_state=model.state_dict(), stats={'train_loss': epoch_loss_rel, 'val_loss': val_loss_rel},
                          val_accs=val_accs, epoch_accs=epoch_accs, optimizer_state=optimizer.state_dict(),
                          additional_reason=reason, regular_save=(epoch + 1) % args.save_epochs == 0)
            if scheduler:
                kwargs['scheduler_state'] = scheduler.state_dict()
            outfile = save_model_state(model_folder, epoch + 1, args, **kwargs)

    if rank == 0:
        end_time = time()
        logging.info(f"training done: total time={end_time - training_start}, "
    ...
    if do_metrics_calculation:
        if rank == 0:
            ...
            inp = None
        metrics = calculate_metrics(args, model, rank=rank, input=inp, device=device, did_training=True, world_size=world_size)
...
def _evaluate(model, val_loader, epoch, rank, device, world_size, val_criterion, args,
              topk=(1, 5), acc_dict_key="acc{}"):
    """Evaluate the model
    ...
    model.eval()
    val_loss = 0
    val_accs = {acc_dict_key.format(k): 0. for k in topk}
    val_start = time()
    n_iters = 0
    iterator = tqdm(val_loader, total=len(val_loader), desc=f"Validating epoch {epoch + 1}") \
        if rank == 0 and args.tqdm else val_loader
...
def _train_one_epoch(model, train_loader, optimizer, rank, epoch, device, mixup, criterion,
                     world_size, scheduler, scaler, args, topk=(1, 5), acc_dict_key="acc{}", show_tqdm=True):
    model.train()
    iterator = tqdm(train_loader, total=len(train_loader), desc=f"Training epoch {epoch + 1}") \
        if rank == 0 and show_tqdm else train_loader

    if not args.amp:
        scaler = NoScaler()

    epoch_loss = 0
    epoch_accs = {acc_dict_key.format(k): 0. for k in topk}
    epoch_start = time()
    grad_norms = []
    n_iters = 0
    for xs, ys in iterator:
        ...
        with torch.cuda.amp.autocast() if args.amp else nullcontext():
            preds = model(xs)
            loss = criterion(preds, ys) + (model.get_internal_loss() if hasattr(model, 'get_internal_loss')
        ...
        if not isfinite(loss.item()):
            ...
            if len(grad_norms) > 0:
                ...
                logging.info(f"Gradient norms until now: min={grad_norms.min().item()}, 20th %tile={torch.quantile(grad_norms, .2).item()}, mean={torch.mean(grad_norms)}, 80th %tile={torch.quantile(grad_norms, .8).item()}, max={grad_norms.max()}")
            sys.exit(1)
    ...
    if len(grad_norms) > 0 and args.gather_stats_during_training:
        ...
        grad_norms = torch.Tensor(grad_norms)
        grad_norm_20 = torch.quantile(grad_norms, .2).item()
        grad_norm_80 = torch.quantile(grad_norms, .8).item()
    ...
    if rank == 0:
        ...
        if args.gather_stats_during_training:
            print_s = f"epoch {epoch + 1}: loss={epoch_loss:.5f}, time={epoch_end - epoch_start}s, " \
                      f"learning rate={lr}"
            for key, val in epoch_accs.items():
                print_s += f", {key}={val*100}%"
            logging.info(print_s)
            if len(grad_norms) > 0:
                logging.info(f"epoch {epoch + 1}: grad norm avrg={grad_norm_avrg}, grad norm max={grad_norm_max}, "
                ...
                logging.warning(f"100% of update steps with infinite grad norms!")
        else:
            logging.info(f"epoch {epoch + 1}: time={epoch_end - epoch_start}s, "
                         f"learning rate={lr}")
